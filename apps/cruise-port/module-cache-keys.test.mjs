...
// Modules whose code changed in this release (0.69.2): app-version.js and ai-support-ui.js (keyboard
...
// Unchanged in 0.69.2, so they keep their earlier keys rather than being bumped for no reason.
...
test('the release is 0.69.2', () => {
  assert.equal(CRUISE_PORT_APP_VERSION, '0.69.2');
});
...
test('the exact release edges: entry → app → UI, unchanged modules keep their keys', () => {
  ...
  for (const name of ['app-version.js', 'ai-support-ui.js']) {
    assert.equal(key('practice-menu-app.js', name), '0.69.2', name);
  }
...
test('no module changed in this release is still requested under an earlier key', () => {
  ...
  for (const name of RELEASE_MODULES) {
    for (const stale of ['0.61.0', '0.62.0', '0.63.0', '0.64.0', '0.65.0', '0.66.0', '0.67.0', '0.68.0', '0.69.0', '0.69.1', '0.59.3', '1.1.8']) {
