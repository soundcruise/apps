...
function assertManifest({ manifestDirectory, expectedId, expectedName, expectedPrefix }) {
    ...
    for (const icon of manifest.icons) {
        assert.match(icon.src, new RegExp(`${expectedPrefix}/icon(?:-maskable)?-(?:192|512)\\.png\\?v=0\\.32\\.0$`));
...
test('each edition references only its formal favicon, Apple icon, and cache-busted manifest', () => {
    assert.match(standardHtml, /<link rel="manifest" href="\.\/manifest\.json\?v=0\.32\.0">/);
    assert.match(standardHtml, /apple-touch-icon[^>]+assets\/app-icons\/standard\/apple-touch-icon-180\.png\?v=0\.32\.0/);
    assert.match(standardHtml, /rel="icon"[^>]+assets\/app-icons\/standard\/favicon-32\.png\?v=0\.32\.0/);
    assert.match(proHtml, /<link rel="manifest" href="\.\/manifest\.json\?v=0\.32\.0">/);
    assert.match(proHtml, /apple-touch-icon[^>]+assets\/app-icons\/pro\/apple-touch-icon-180\.png\?v=0\.32\.0/);
    assert.match(proHtml, /rel="icon"[^>]+assets\/app-icons\/pro\/favicon-32\.png\?v=0\.32\.0/);
