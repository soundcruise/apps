...
for (const platform of ['ios', 'android', 'macos', 'windows', 'web']) assert.match(markup, new RegExp(`id="my-apps-${platform}-url"`));
assert.match(markup, /id="my-apps-other-urls"[^>]*>[\s\S]*?PC・その他のURL/);
assert.match(markup, /専用URLが設定されていない端末では、共通URLを使用します/);
assert.match(markup, /id="my-apps-url-help-close"[^>]+aria-label="URLの説明を閉じる"/);
...
assert.match(styles, /\.my-apps-other-urls-body[\s\S]*padding/);
