...
test('card label and tab mapping keep sold separate', () => {
    assert.equal(gearCardLabel(base), 'YAMAHA / LL6');
    assert.equal(gearCardLabel({ ...base, manufacturer: '' }), 'LL6');
    assert.deepEqual(GEAR_TABS.map(({ status }) => status), ['all', 'wishlist', 'owned', 'sold']);
});

test('grid preference accepts 1, 2, and 3 per device', () => {
    ...
    assert.equal(saveGearGridColumns(4, storage), false);
    assert.equal(gearGridColumns(storage), 3);
});
...
test('CSV exports only current tab/category and escapes text and formulas', () => {
    ...
    assert.equal(gearItemsForExport(items, 'owned').length, 1);
