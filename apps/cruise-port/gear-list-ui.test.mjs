...
assert.doesNotMatch(markup, /id="gear-sold-tab"/);
assert.match(markup, /data-gear-columns="1"[\s\S]*?data-gear-columns="2"[\s\S]*?data-gear-columns="3"[\s\S]*?data-gear-columns="4"/);
...
assert.match(markup, /class="gear-list-title-row"[\s\S]*?id="wishlist-title"[\s\S]*?id="gear-list-title-add"[^>]*>＋ 機材を追加/);
assert.match(proMarkup, /class="gear-list-title-row"[\s\S]*?id="wishlist-title"[\s\S]*?id="gear-list-title-add"[^>]*>＋ 機材を追加/);
...
assert.match(gearFormMarkup, /id="gear-manufacturer"[\s\S]*?id="gear-name"/);
...
assert.match(source, /gearManufacturerInput/);
assert.doesNotMatch(source, /gearUrlInput|formatGearPrice/);
assert.doesNotMatch(gearSource, /item\.url|item\.priceYen/);
...
assert.match(styles, /\.gear-category-toolbar[\s\S]*?position: relative/);
assert.match(styles, /\.gear-category-select[\s\S]*?flex: 1 1 auto[\s\S]*?min-width: 0[\s\S]*?height: 44px[\s\S]*?font-size: calc\(1rem \* var\(--font-scale\)\)/);
assert.match(styles, /\.gear-category-menu-toggle[\s\S]*?width: 44px[\s\S]*?height: 44px/);
assert.match(styles, /\.gear-category-menu[\s\S]*?position: absolute[\s\S]*?z-index: 40[\s\S]*?background: #171611/);
assert.match(styles, /\.gear-category-menu-item[\s\S]*?grid-template-columns: 20px minmax\(0, 1fr\)[\s\S]*?height: 44px/);
assert.match(styles, /\.gear-category-menu-item:disabled/);
assert.match(styles, /\.gear-list-title-add[\s\S]*?min-height: 44px/);
...
assert.match(styles, /\.gear-card-actions[\s\S]*?grid-template-columns: 1fr/);
