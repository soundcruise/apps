...
assert.match(markup, /id="gear-list-form"[\s\S]*?id="gear-name"[\s\S]*?id="gear-category"[\s\S]*?id="gear-manufacturer"[\s\S]*?id="gear-url"[\s\S]*?id="gear-price"[\s\S]*?id="gear-status"[\s\S]*?id="gear-priority"[\s\S]*?id="gear-memo"/);
assert.match(markup, /maxlength="100"/);
assert.match(markup, /id="gear-memo"[\s\S]*?maxlength="1000"/);
assert.match(markup, /id="gear-price"[\s\S]*?min="0"[\s\S]*?step="1"/);
...
assert.match(store, /GEAR_LIST_SCHEMA_VERSION = 1/);
...
assert.match(source, /link\.textContent = '商品ページを開く'/);
