...
import {
    ...
    savePracticeMenus,
    updatePracticeMenu
} from './practice-menu-store.js?v=0.59.2';
import {
...
import { applyHomeCruiseLinks, bindHomeCruiseLaunch } from './cruise-app-links.js?v=0.59.2';
import {
    SYNC_CENTER_ROUTE,
    createSyncCenterController,
    readSyncCenterConfig
} from './sync-center-controller.js?v=0.59.2';
import { bindSyncCenterActions, renderSyncCenter } from './sync-center-ui.js?v=0.59.2';
import { createPortSyncStatus } from './port-sync-status.js?v=0.59.2';
import { createSyncCenterOrchestrator } from './sync-center-orchestrator.js?v=0.59.2';
import {
    openSyncCenter,
    restoreInitialSyncCenterRoute,
    returnToSyncCenterSource
} from './sync-center-navigation.js?v=0.59.2';
import {
...
} from './practice-menu-progress-store.js?v=0.59.2';
import {
    ...
    savePracticeHistory,
    toLocalDateKey
} from './practice-menu-history-store.js?v=0.59.2';
...
} from './practice-menu-calendar-store.js?v=0.59.2';
import {
    ...
    startPracticeTimer,
    stopPracticeTimer
} from './practice-menu-timer-store.js?v=0.59.2';
import { buildPracticeAnalytics, formatPracticeTotal } from './practice-analytics.js?v=0.59.2';
import {
...
import {
    ...
    saveMyApps,
    validateMyAppValues
} from './my-apps-store.js?v=0.59.2';
import {
    getKnownApp,
...
import {
    ...
    resolveMyAppHref
} from './my-apps-launch.js?v=0.59.2';
import {
...
} from './app-version.js?v=0.59.2';
import { applyHomeDisplaySize, applyHomeSectionOrder } from './home-display.js?v=0.25.0';
import { DEFAULT_SETTINGS, moveHomeSection, clearRetiredIconScalePreviewKeys, loadSettings, saveSettings } from './settings-store.js?v=0.59.2';
import { initTuner } from './tuner-app.js?v=0.59.2';
import {
    ...
    updateGearItem,
    validateGearValues
} from './gear-list-store.js?v=0.59.2';
import { gearCardLabel, gearGridColumns, saveGearGridColumns, gearItemsForExport, gearListCsv } from './gear-list-ux.js?v=0.59.2';
import { createGearCardPng, shareGearCardPng } from './gear-card-export.js?v=0.59.2';
import { createGearListPng } from './gear-list-image-export.js?v=0.59.2';
import {
    ...
    renameGearCategory,
    saveGearCategories
} from './gear-category-store.js?v=0.59.2';
import { createGearPhotoStore } from './gear-photo-store.js?v=0.27.0';
import { PortAssetSync } from './port-asset-sync.js?v=0.59.2';
import { validatePortLocalCollections } from './port-sync-local-validation.js?v=0.59.2';
import {
...
function stopPracticeTimerWithHistory(now = new Date()) {
    ...
    const savedHistory = savePracticeHistory(historyResult.history, undefined, { baseHistory: previousHistory });
    if (!savedHistory.ok) {
        return { ok: false, message: '練習記録を保存できませんでした。タイマーは継続しています。' };
    }
    if (!savePracticeTimer(transition.timer).ok) {
        if (!savedHistory.history) savePracticeHistory(previousHistory);
        else state.history = savedHistory.history;
        return { ok: false, message: 'タイマーを終了できませんでした。計測を継続しています。' };
    }
    state.history = savedHistory.history || historyResult.history;
...
function refreshVisibleSyncCenterStatus() {
    ...
    syncCenterStatusRefreshQueued = true;
    queueMicrotask(() => {
        syncCenterStatusRefreshQueued = false;
        if (location.hash === SYNC_CENTER_ROUTE) void renderSyncCenterView();
    });
}
...
async function renderSyncCenterView() {
    ...
    try {
        if (await syncCenterOrchestrator.hasConfiguredAccount?.()) {
    ...
    renderSyncCenter(elements.syncCenterView, {
        ...
        portStatus: createPortSyncStatus({
            accountState: presentation.accountState,
...
function renderGearCard(item) {
    ...
    else card.classList.add('gear-card--no-photo');
    ...
    const metadata = createGearMetadata(item);
    ...
    metadata.append(menuToggle, actions);
    card.append(metadata);
    return card;
}

function renderGearCategoryFilter() {
    const categories = [{ id: 'all', name: '全てのカテゴリ' }, ...gearState.categories];
...
window.addEventListener('cruise-port-sync-state', refreshVisibleSyncCenterStatus);
window.addEventListener('cruise-port-asset-sync-state', refreshVisibleSyncCenterStatus);
