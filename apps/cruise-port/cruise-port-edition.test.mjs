...
test('SP2 shells allow only Pro title/gate and document-relative URL differences', () => {
    ...
    const withoutGate = pro
        ...
        .replace('href="./manifest.json?v=0.51.0"', 'href="../manifest.json?v=0.51.0"')
    ...
    for (const entry of [root, pro]) {
        ...
        assert.match(entry, /port-sync-adapter\.js\?v=0\.51\.0/);
        assert.match(entry, /port-sync-controller\.js\?v=0\.51\.0/);
