...
test('SP2 shells allow only Pro title/gate and document-relative URL differences', () => {
    ...
    const withoutGate = pro
        ...
        .replace('href="./manifest.json?v=0.45.10"', 'href="../manifest.json?v=0.45.10"')
    ...
    for (const entry of [root, pro]) {
        ...
        assert.match(entry, /port-sync-adapter\.js\?v=0\.45\.10/);
        assert.match(entry, /port-sync-controller\.js\?v=0\.45\.10/);
