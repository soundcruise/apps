...
test('SP2 shells allow only Pro title/gate and document-relative URL differences', () => {
    ...
    const withoutGate = pro
        ...
        .replace('href="./manifest.json?v=0.44.4"', 'href="../manifest.json?v=0.44.4"')
    ...
    for (const entry of [root, pro]) {
        ...
        assert.match(entry, /port-sync-adapter\.js\?v=0\.44\.4/);
        assert.match(entry, /port-sync-controller\.js\?v=0\.44\.4/);
