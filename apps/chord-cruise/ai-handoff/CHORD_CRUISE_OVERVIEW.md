...
- 現在のバージョン: `0.14.0`（`index.html` の各`<script>`タグの `?v=`、および `js/app.js` 内 `CHORD_CRUISE_APP_VERSION`）
- 最新commit（chord-cruise関連、`git log --oneline -- apps/chord-cruise/` で確認）:
  - hash: `8359157c`
  - message: `コードクルーズの設定とハイフレット表示を拡張`
...
| `js/ui/save-editor.js` | フォームの保存前編集と保存コード編集の共通モーダル。保存範囲・表示モード・運指・名前・メモ・フォルダを編集し、保存コードは同一IDでの上書き／新規IDでの別名保存を選べる。保存コード編集の初回だけ押弦範囲を中央寄せする。 |
...
- **コード本棚画面**（`cc-screen-library`）: `js/ui/library.js`が「フォルダ一覧 → 指板サムネイル一覧 → 保存コード詳細」の3階層を動的に描画する。一覧はコード名と保存範囲だけの指板を1〜4列で表示し、列数はsettingsへ保存する。詳細は指板直上へコード名だけを表示し、一時的な白黒表示、PNG書き出し、保存コード編集に対応する。
...
- **コード本棚**: 保存したコードフォームをフォルダ単位で整理し、一覧ではコード名＋軽量SVG指板を1〜4列で表示する。詳細・一覧・書き出しは同じ保存範囲描画モデルを使い、フレット番号は背景帯なし。保存済みデータは複製して編集を始め、上書き確定まで元レコードを変更しない。白黒図は背景白、左右境界あり、1弦／6弦を上下端とし、ポジションマークを表示しない。ミュート×は先頭表示列中央へ置く。
...
- バージョン定数: `js/app.js` 内 `CHORD_CRUISE_APP_VERSION`（現在 `0.14.0`）。
- `?v=` によるキャッシュ管理: `index.html` 内の全13本の `<script src="...?v=0.14.0">` タグ、および `<link rel="stylesheet" href="theme.css?v=0.14.0">` が同じバージョン文字列を共有している。
...
コード本棚の列数設定、一覧／詳細コード名、編集初回スクロール、白黒指板とミュート表示、特大フレット番号を追加し、マイナーバージョンとして `0.14.0` へ更新した。settingsへ`libraryColumns`を追加したが、保存コードとlocalStorageのschemaVersionは`1`のまま変更していない。
...
| `index.html`内13本のscriptタグの`?v=`更新漏れ | バージョンを上げる際は`grep -n "?v=" index.html`で全箇所を確認する。 |
