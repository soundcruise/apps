...
- 正式Chord Cruise HEAD: `7e5c2154f901ea4338caf485a1da83ae28e761bb`
...
- **7(no5) / m7(no5) CAGED候補（未commit）**: `7no5`（`[0,4,10]`／`1 3 ♭7`／`C7(no5)`）と`m7no5`（`[0,3,10]`／`1 ♭3 ♭7`／`Cm7(no5)`）を追加し、既存7th／m7の5度slotだけを除外したC/A/G/E/D固定FORMを候補として扱う。root・3度／♭3・♭7の高音側slotを含む全残存FORM音を保持し、`deletedNotes`／`pendingDelete`は使わない。初期FINGERINGは推測せず`fingeringStatus: 'undefined'`とし、保存前編集で個別指定できる。schemaVersion 1・migration・versionは不変で、実機確認後に正式化する。
