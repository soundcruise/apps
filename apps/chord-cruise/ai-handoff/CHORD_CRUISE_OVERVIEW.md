...
- 現在のバージョン: `0.11.0`（`index.html` の各`<script>`タグの `?v=`、および `js/app.js` 内 `CHORD_CRUISE_APP_VERSION`）
- 最新commit（chord-cruise関連、`git log --oneline -- apps/chord-cruise/` で確認）:
  - hash: `6b32c65e`
  - message: `コードクルーズの減三和音表記を変更`
...
- JS/CSSの共有関係:
  ...
  - JSは1本の `script.js` ではなく、`js/core/*.js`（データ・ロジック層）と `js/ui/*.js`（画面描画層）に分割されたモジュール構成。`index.html` から12本のscriptタグで個別に読み込む（`window.ChordCruise` というグローバルオブジェクトに各モジュールが機能を生やす形）。
...
| `js/ui/fretboard.js` | コードクルーズ専用の指板描画。実フレット範囲を受け取り、0〜13F／12〜25F（保存詳細では必要に応じた範囲）を同じ列幅で描画する。番号強調とポジションドットは別ロジック。 |
...
- **CAGEDフォーム表示**: `js/core/caged-forms.js` の辞書データに基づき、指定された実フレット範囲内でフォームを探索する。12〜25Fでは通常フォームを+12Fへ移し、`openFingers` ではなくムーバブル運指を使う。
- **押さえ方・運指・バレーコード表示の注意点**: 運指はCAGEDフォーム選択中のみ有効。同じ指・同じ実フレットのノートからバレーを導出し、12F以上でも同じ判定を使う。保存前編集と本棚詳細では音タップで運指を変更できる。
...
- バージョン定数: `js/app.js` 内 `CHORD_CRUISE_APP_VERSION`（現在 `0.11.0`）。
- `?v=` によるキャッシュ管理: `index.html` 内の全12本の `<script src="...?v=0.11.0">` タグ、および `<link rel="stylesheet" href="theme.css?v=0.11.0">` が同じバージョン文字列を共有している。
- 通常版/PRO版で更新箇所が分かれているか: PRO版が存在しないため該当なし。
- service workerの更新: service worker自体が存在しないため不要。
- **バージョン更新漏れしやすい箇所**: `index.html`内の12本のscriptタグすべてに同一の`?v=`が付いているため、1本でも更新し忘れるとキャッシュ不整合が起きる可能性がある。バージョンを上げる際は、`grep -n "?v=" index.html` で全箇所を確認してから一括更新すること。

ハイフレット表示と番号強調設定を含むユーザー向け機能変更では、マイナーバージョンとして `0.11.0` へ更新した。
...
| `index.html`内12本のscriptタグの`?v=`更新漏れ | バージョンを上げる際は`grep -n "?v=" index.html`で全箇所を確認する。 |
