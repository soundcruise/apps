...
- 現在のバージョン: `0.28.2`（Phase G5-D M7(no5) CAGED対応を正式化）
...
- バージョン定数: `js/app.js` 内 `CHORD_CRUISE_APP_VERSION`（現在 `0.28.2`）。
- `?v=` によるキャッシュ管理: `index.html` 内の全15本のscriptタグとstylesheet link（計16参照）が同じ `0.28.2` を共有している。
