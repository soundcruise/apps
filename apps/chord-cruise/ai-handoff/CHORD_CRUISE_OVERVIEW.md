...
- 現在のバージョン: `0.27.1`（omit/no表記整理を正式化）
...
- バージョン定数: `js/app.js` 内 `CHORD_CRUISE_APP_VERSION`（現在 `0.27.1`）。
- `?v=` によるキャッシュ管理: `index.html` 内の全15本のscriptタグとstylesheet link（計16参照）が同じ `0.27.1` を共有している。
