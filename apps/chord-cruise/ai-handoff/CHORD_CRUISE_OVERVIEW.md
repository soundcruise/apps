...
- 現在のバージョン: `0.37.9`（CAGED設定・保存フロー改善）
...
- 保存制限: Standardはフォルダ最大3個・1フォルダ最大10コード、Proは無制限
- Pro導線: StandardのPro案内 → `pro-access.html` → Pro入口 → shared gate
- 設定表示: Standardは「Pro版はこちら」、Proは「Pro版を利用中」
- 保存互換: `schemaVersion: 1`、`qualityKey`、`intervals`、`bassPc`を維持し、edition情報は保存しない
...
- **7sus4 / M7sus4 CAGED候補（未commit）**: `7sus4`（`[0,5,7,10]`／`1 4 5 ♭7`）と`M7sus4`（`[0,5,7,11]`／`1 4 5 7`）を追加し、既存7th／M7の3度slotだけを4度へ置換したC/A/G/E/D固定FORMを候補として扱う。Sus4 FORMへ7thをruntime合成せず、qualityごとのslot・mute・warningを独立させる。初期FINGERINGは推測せず`finger:null`のまま、FORM定義の`fingeringStatus: 'undefined'`で「運指未定義」と既存の`⚠️運指`を区別する。FORM全音をCDE／ドレミ／度数へ保持し、保存前編集で個別に指を指定できる。保存schema・migration・versionは不変で、実機確認後に運指を決定する。
...
- **v0.32.0 Phase G7-C-1 7♭5 CAGED正式化**: `7b5` quality（`[0,4,6,10]`／`1 3 ♭5 ♭7`／`C7♭5`）を追加し、既存7thのinterval 7（5度）slotだけをinterval 6（♭5）へ置換したC/A/G/E/D固定FORMを正式化した。altered fifthはoverlayやtensionではなくコードcoreとして管理し、root・3度・♭7と高音側slotは保持する。初期FINGERINGは推測せず`fingers: {}`と`fingeringStatus: 'undefined'`で保存前編集を案内する。schemaVersion 1・migration・versionは0.32.0へ更新。正式HEADは`e7d41073609740962a3fa686fbcaf860daa91813`。
- **v0.33.0 Phase G7-C + G7-D正式化**: `7b5`（`[0,4,6,10]`）、`maj7b5`（`[0,4,6,11]`）、`7no3`（`[0,7,10]`）、`maj7no3`（`[0,7,11]`）をqualityとして追加し、既存7th／M7 CAGEDからaltered fifthまたは3度のみを変更・除外したC/A/G/E/D固定FORMを正式化した。root・残存構成音・高音側slotを保持し、runtime filterやoverlayは使わない。全qualityの初期FINGERINGは`fingeringStatus: 'undefined'`で保存前編集を案内し、spell・save/reload・library・SVG/PNGを確認した。schemaVersion 1・migrationなし。実装commit・正式HEADは`d53a9c5a50d194756d7429b4d5edb47b0f8f568f`。
- **Phase G8-C-2 QUALITIES metadata候補（未commit）**: 既存25 qualityの理論情報（intervals／degreeLabels／suffix／symbolSuffix／romanSuffix）を不変のまま、一覧・CAGED対応表示の将来基盤として`family`・`modifier`・`caged` metadataを追加した。familyはmajor／minor／dominant／diminished／augmented／sus／power、modifierはnone／sixth／no／alteredとし、cagedは`{ supported, mode, baseQuality }`で表す。全qualityが現時点ではCAGED対応済みで、23 qualityは`fixed`、`6`はMajor・`m6`はMinorを基底とする`overlay`。edition、identifyQuality、generateName、CAGED lookup、overlay、UI、保存schema、migration、versionは変更しない。
- **v0.34.0 Phase G8-D-4正式化**: QUALITIESへ`family`・`modifier`・`caged` metadataを追加し、25 qualityの理論情報・identifyQuality・CAGED lookup・spell engine・保存schema・migrationは維持した。`6`／`m6`はそれぞれMajor／Minor基底のoverlay、その他はfixed FORMとして分類した。CAGED未対応時の内部向け文言を「このコードはCAGEDフォーム未対応のため、型フォームを表示していません。」へ整理し、既存の全体表示・範囲外・運指未定義・⚠️運指の文言は維持した。Pro gate・edition・UI構造は追加していない。実装commit・正式HEADは`f491ac9649da52864de7f5942b8510deb3e5eba6`、docs commitは後続記録予定。
- **v0.35.0 Phase P7 Chord Cruise Pro正式化**: `feature-access.js`と`data-app-edition="Pro"`のPro入口を追加し、既存のshared Pro gateへ接続した。Standardでは任意コードの作成・構成音・通常指板表示を維持しつつ、`customChordSave`、`unlimitedLibrary`、`advancedQuality`、`advancedCaged`を独立制御する。保存境界でカスタムフォルダ3個・1フォルダ10コードを検査し、上書きと既存record閲覧は維持する。QUALITIESへedition非依存の`complexity`を追加し、高度qualityの詳細分析とCAGEDフォームだけをPro案内へ切り替える。保存schemaVersion 1・migration・intervals・qualityKey・bassPcは不変。通常保存時に任意コード用Pro案内が見えるCSS競合は`[hidden]`優先で修正した。実装commit・正式HEADは`ecd4ad09b760e766791b6a4150a6297ab540b744`。
- **Phase G7-C-2 M7♭5 CAGED候補（未commit）**: `maj7b5` quality（`[0,4,6,11]`／`1 3 ♭5 7`／`CM7♭5`）を追加し、既存M7のinterval 7（5度）slotだけをinterval 6（♭5）へ置換したC/A/G/E/D固定FORMを候補として扱う。altered fifthはoverlayやtensionではなくコードcoreとして管理し、root・3度・7度と高音側slotは保持する。C型だけは開放Gを♭5へ半音下降すると負フレットになるため、同じ3弦の♭5を1オクターブ上へ置いて既存表示域でも保持する。初期FINGERINGは推測せず`fingers: {}`と`fingeringStatus: 'undefined'`で保存前編集を案内する。schemaVersion 1・migration・versionは不変。
- **Phase G7-D-1 7(no3) CAGED候補（未commit）**: `7no3` quality（`[0,7,10]`／`1 5 ♭7`／`C7(no3)`）を追加し、既存7thのinterval 4（3度）slotだけを除外したC/A/G/E/D固定FORMを候補として扱う。root・5度・♭7と高音側slotは保持し、除外した3度担当弦だけをmuteにする。runtime filterやoverlayは使わず、既存no5系と同じ固定FORM方式で管理する。初期FINGERINGは推測せず`fingers: {}`と`fingeringStatus: 'undefined'`で保存前編集を案内する。schemaVersion 1・migration・versionは不変。
- **Phase G7-D-2 M7(no3) CAGED候補（未commit）**: `maj7no3` quality（`[0,7,11]`／`1 5 7`／`CM7(no3)`）を追加し、既存M7のinterval 4（3度）slotだけを除外したC/A/G/E/D固定FORMを候補として扱う。root・5度・7度と高音側slotは保持し、除外した3度担当弦だけをmuteにする。runtime filterやoverlayは使わず、既存no5系と同じ固定FORM方式で管理する。初期FINGERINGは推測せず`fingers: {}`と`fingeringStatus: 'undefined'`で保存前編集を案内する。schemaVersion 1・migration・versionは不変。
...
- **Phase G6-C-1 / C-2 pitch spelling候補（未commit）**: `spellChordNotes()`はroot／quality／interval／degree／keyContextから任意コードの表示用音名を理論綴りで返し、既存の`noteName()`・sharp/flat配列は維持する。C-2ではExploreの任意コード全体表示とsave-editorプレビューのCDE／ドレミラベルへ適用し、`Cm`を`C E♭ G`／`ド ミ♭ ソ`、`C7`を`C E G B♭`／`ド ミ ソ シ♭`のように表示する。quality定義→degree→interval推定→既存表示へのfallbackを持つ。CAGED FORM markerの独立経路、sixth／tension／bass overlay、本棚、SVG/PNG、保存schema・migration・versionは今回変更しない。
- **Phase G6-C-3 pitch spelling候補（未commit）**: CAGED FORM marker、6th overlay、tension overlayも`spellChordNotes()`結果からCDE／ドレミラベルを取得する。markerの座標・interval・degree・role・overlayTypeは不変で、C6／Cm6のinterval 9は`A`／`ラ`かつ`degree: 6`・`role: 'sixth'`のまま、C7(9)・Cadd9・C11・C13の追加音はそれぞれ`D`・`D`・`F`・`A`として表示する。bass overlay、本棚、SVG/PNG、schema・migration・versionは変更しない。
- **Phase G6-C-4 pitch spelling候補（未commit）**: 本棚・保存コード詳細・静的SVG・PNG元SVGも、保存recordの`rootPc`／`qualityKey`／`intervals`／`tensionPcs`／`keyContext`から`spellChordNotes()`用のdegreeを復元して音名・ドレミを統一する。`chordName`は既存保存表示・PNGタイトルとして維持するが、音名綴りの解析には使わない。schemaVersion 1、intervals、qualityKey、notes、migrationは不変。Bass overlayの既存`bassPc`表示経路も維持する。
...
- **PRO導線**: Standardの保存・高度quality・高度CAGEDの案内から`pro-access.html`へ進み、Pro入口`pro_k7m4q9v2x8/index.html`でshared gateを通過する。設定画面にもStandard向け「Pro版はこちら」またはPro向け「Pro版を利用中」を表示する。
...
- 高度qualityの詳細分析は`advancedQuality`、高度qualityのCAGEDフォームは`advancedCaged`でPro制御する。CAGED未対応や運指未定義の文言とは分離する。
- 保存recordはStandard/Proで共通で、`schemaVersion: 1`、`qualityKey`、`intervals`、`bassPc`を維持し、edition情報を保存しない。
...
- **通常版/PRO版で差がある機能**: `customChordSave`、`unlimitedLibrary`、`advancedQuality`、`advancedCaged`。作成・確認・通常指板表示はStandardにも残す。
- **触る時に注意すべきロジック**:
...
- バージョン定数: `js/app.js` 内 `CHORD_CRUISE_APP_VERSION`（現在 `0.37.9`）。
...
7. Standard/Proの相対パス、`data-app-edition`、shared gate参照を確認する
