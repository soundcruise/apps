...
- 現在のバージョン: `0.13.0`（`index.html` の各`<script>`タグの `?v=`、および `js/app.js` 内 `CHORD_CRUISE_APP_VERSION`）
...
| `js/ui/save-editor.js` | フォームの保存前編集と保存コード編集の共通モーダル。保存範囲・表示モード・運指・名前・メモ・フォルダを編集し、保存コードは同一IDでの上書き／新規IDでの別名保存を選べる。 |
...
- **コード本棚画面**（`cc-screen-library`）: `js/ui/library.js`が「フォルダ一覧 → 指板サムネイル一覧 → 保存コード詳細」の3階層を動的に描画する。一覧は文字情報を置かず保存範囲だけの指板を常時4列で表示する。詳細は保存範囲の列だけを表示し、一時的な白黒表示、コード名だけを添えるPNG書き出し、保存コード編集に対応する。
...
- **コード本棚**: 保存したコードフォームをフォルダ単位で整理し、一覧では文字情報を省いた4列の軽量SVG指板として表示する。詳細・一覧・書き出しは同じ保存範囲描画モデルを使い、フレット番号は背景帯なし。保存済みデータは複製して編集を始め、上書き確定まで元レコードを変更しない。白黒時は押弦を黒丸＋白文字、開放弦を白丸＋黒文字にする。
...
- バージョン定数: `js/app.js` 内 `CHORD_CRUISE_APP_VERSION`（現在 `0.13.0`）。
- `?v=` によるキャッシュ管理: `index.html` 内の全13本の `<script src="...?v=0.13.0">` タグ、および `<link rel="stylesheet" href="theme.css?v=0.13.0">` が同じバージョン文字列を共有している。
...
コード本棚の指板一覧・白黒表示・PNG書き出しを追加した未コミットの`0.12.0`変更へ、4列一覧、保存コード編集、番号帯削除、画像整理を加え、マイナーバージョンとして `0.13.0` へ更新した。localStorageのschemaVersionは`1`のまま変更していない。
