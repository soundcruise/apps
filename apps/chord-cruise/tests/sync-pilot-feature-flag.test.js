...
(async function () {
    ...
    assert(freshProduction.appended.some(function (url) { return url.endsWith('/sync-client.js?v=1.10.4'); }),
    ...
    assert(proHtml.includes('../js/sync/sync-turnstile.js?v=1.10.4'), 'Pro loads the production Turnstile provider');
    assert(proHtml.includes('../js/sync/sync-bootstrap.js?v=1.10.4'), 'Pro loads the OFF-first bootstrap');
    ...
    assert(source.indexOf("loadScript('sync-merge.js')") < source.indexOf("loadScript('sync-client.js?v=1.10.4')"));
