...
- **D2 MVP（完了・v0.17.0。push済み: `ac8acc96`）**: 小節上に出ていた極小overlayを
  ...
  データ構造・保存/JSON/プレDTM/MIDI経路は無変更、`schemaVersion` は1のまま
- **D3a（完了・v0.18.0。ローカルcheckpoint: `a3a1618f`。origin/mainへは未push）**:
  ...
  データ構造・保存/JSON/プレDTM/MIDI経路は無変更、`schemaVersion` は1のまま
- **F1（実装・検証済み・v0.19.0。commit/push未実施）**: フローティング小節ルーペ化
...
- **F2（未着手）**: 歌詞のtick位置セル編集（`setBarLyricSlot` 相当）
- **F3（未着手）**: ドレミのtick位置セル編集（`setBarMelodySlot` 相当）
- **F4（未着手）**: まとめて入力の自動分解配置（歌詞・ドレミをセルへ流し込む）、
  旧・小節グリッドカードの撤去（`ai-handoff/PROJECT_STATE.md` 9章のtick:0潰れ問題に対応）、
  UI全体の整合仕上げ
