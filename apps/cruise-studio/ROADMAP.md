...
段階的にUIを再編する。VexFlow / 五線譜 / MusicXML（Phase 6）はこのPhase 3.5完了後に着手する。
...
- **D2 MVP（完了・v0.17.0。push済み: `ac8acc96`）**: 小節上に出ていた極小overlayを
  ...
  データ構造・保存/JSON/プレDTM/MIDI経路は無変更、`schemaVersion` は1のまま
- **D3a（実装・検証済み・v0.18.0。commit/push未実施）**: 「選択小節のX線写真」方針
  （Fable5レビュー確定）。D2の巨大な縦積みフォーム型ドックを、選択小節と同じ時間軸を
  保ったまま横に拡大するコンパクトなタイムグリッド・ドック（高さ約200〜220px）へ変更。
  拍ルーラー、コード（1小節1セル）/ストローク/歌詞/ドレミの共通時間軸配置、
  ヘッダー1行集約（8分/16分切替・まとめて入力ポップオーバー化・基本に戻す）、
  選択小節の自動スクロールを追加。**本格的にセル編集するのはストローク行のみ**
  ...
  移植はまだ行わない。データ構造・保存/JSON/プレDTM/MIDI経路は無変更、
  `schemaVersion` は1のまま
- **D3b（未着手）**: 歌詞・ドレミのtick位置セル編集、ストロークセルと同等の
  完全なキーボード操作（行間移動等）への拡張。旧・小節グリッドカードとの
  データ競合（tick:0への潰れ）に対応する必要あり（`ai-handoff/PROJECT_STATE.md` 9章参照）
- **D3c（未着手）**: まとめて入力の自動分解配置（歌詞・ドレミをセルへ流し込む）、
  旧・小節グリッドカードの撤去、UI全体の整合仕上げ

**完了条件**: 譜面が常に主役として大きく表示され、下部ドックから小節の主要な入力
（コード・ストローク・歌詞・ドレミ）が完結できる。旧overlay編集・旧一覧編集UIへの
依存を段階的に下げられる状態になる。
