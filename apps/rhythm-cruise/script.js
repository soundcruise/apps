...
/* 判定のきびしさプリセット（v0.9.164）。GOOD/EARLY/LATE の「窓（幅）」だけを切り替える。
   5段階：やさしい / 標準 / やや厳しい / 厳しい / もっと厳しい。初期値・未保存・不正値は standard。
   ...
   ・nearFrac<0.5 のときは、最寄り拍へ割り当てた入力でも「拍間隔×nearFrac〜拍間隔×0.5」の帯は
     ...
     「もっと厳しい」(GOOD±15ms / frac0.25)は上級者向けにかなりシビアになる。 */
const JUDGE_PRESETS = {
...
/* 不正値・未保存は standard へフォールバック（既存ユーザーの strict はそのまま「厳しい」扱い）。 */
function normalizeJudgePreset(v) {
    return (v === 'easy' || v === 'semiStrict' || v === 'strict' || v === 'veryStrict') ? v : 'standard';
}
...
const state = {
    ...
    judgePreset: 'standard', // 判定のきびしさ（v0.9.164）：'easy' | 'standard' | 'semiStrict' | 'strict' | 'veryStrict'
...
const JUDGE_PRESET_LABELS = { easy: 'やさしい', standard: '標準', semiStrict: 'やや厳しい', strict: '厳しい', veryStrict: 'もっと厳しい' };
...
function updateJudgePresetUI() {
    ...
    if (els.judgePresetSeg) {
        els.judgePresetSeg.querySelectorAll('.ss-seg').forEach((b) => {
...
function bind() {
    ...
    // 判定のきびしさ（v0.9.164）：やさしい / 標準 / やや厳しい / 厳しい / もっと厳しい
    if (els.judgePresetSeg) els.judgePresetSeg.querySelectorAll('.ss-seg').forEach((b) => {
