...
/* 確認音バランス（クリック↔ストローク）スライダーの 0〜100 値を取得/反映する共通ヘルパー（v0.9.247）。
   プリセット保存時に現在のスライダー値を取り込み、呼び出し時に復元するために使う。
   内部の rhythmPreviewSoundBalance は -1〜+1。スライダーは 0〜100（50=中央）。 */
function clampRhythmBalanceSliderValue(v) {
    const n = Math.round(Number(v));
    return Number.isFinite(n) ? Math.max(0, Math.min(100, n)) : 50;
}
function readRhythmBalanceSliderValue(slider) {
    if (slider && Number.isFinite(Number(slider.value))) return clampRhythmBalanceSliderValue(slider.value);
    return clampRhythmBalanceSliderValue((rhythmPreviewSoundBalance + 1) * 50);
}
function applyRhythmBalanceSliderValue(slider, value) {
    const v = clampRhythmBalanceSliderValue(value);
    if (slider) slider.value = String(v);
    setRhythmPreviewSoundBalanceFromValue(v);
}
...
const CHORD_COOLDOWN_MIN_MS = 100;
/* 100ms下限がgap×45%を上回る高速域。16分/32分の本格対応はコード専用判定幅と併せて検討する。 */
const CHORD_FAST_GAP_WARN_MS = 225;
...
/* 判定のきびしさプリセット（v0.9.164）。GOOD/EARLY/LATE の「窓（幅）」だけを切り替える。
   ...
   ・実際のEARLY/LATE窓は max(nearMs, 拍間隔×nearFrac)。速いテンポでは nearMs 床が効いて隣拍に吸われすぎず、
     遅いテンポでは拍間隔×nearFrac で頭打ちにして広がりすぎを防ぐ。
   ・nearFrac<0.5 のときは、最寄り拍へ割り当てた入力でも「拍間隔×nearFrac〜拍間隔×0.5」の帯は
     タイミングMISSになり得る（＝少しのズレでMISSが出る）。きびしい側ほどこの帯が広がり、
     右端(GOOD±15ms / frac0.25)は上級者向けにかなりシビアになる。 */
const JUDGE_PRESETS = {
    easy: { justMs: 45, nearMs: 140, nearFrac: 0.55 },
    standard: { justMs: 37, nearMs: 115, nearFrac: 0.48 },
    semiStrict: { justMs: 30, nearMs: 95, nearFrac: 0.40 },
    strict: { justMs: 23, nearMs: 75, nearFrac: 0.32 },
    veryStrict: { justMs: 15, nearMs: 55, nearFrac: 0.25 },
};
/* 不正値・未保存は semiStrict へフォールバック（保存済みの standard/strict などはそのまま尊重）。 */
function normalizeJudgePreset(v) {
    return (v === 'easy' || v === 'standard' || v === 'semiStrict' || v === 'strict' || v === 'veryStrict') ? v : 'semiStrict';
}
...
const state = {
    ...
    judgePreset: 'semiStrict', // 判定のきびしさ（v0.9.164）：'easy' | 'standard' | 'semiStrict' | 'strict' | 'veryStrict'
    ...
    practiceRecording: {
        ...
        playbackPaused: false,           // v0.10.43：一時停止中（位置・JUSTライン・debug候補を保持）
        playbackPausePositionSec: 0,     // v0.10.43：一時停止したバッファ内位置（秒）
        ...
        // v0.10.32：録音再生クリック重ね（録音振り返り専用の一時state。localStorage/履歴/判定には混ぜない）
        reviewClickEnabled: true,        // クリックを重ねるON/OFF（既定ON。Bluetooth対策が主目的）
        reviewClickVolume: 50,           // クリック音量(%)。state.clickVolumeとは別管理（既定=REVIEW_CLICK_VOLUME_DEFAULT）
        reviewClickTimes: [],            // {t:録音開始基準の相対秒, accent} の配列（state.clickTimesのコピー）
...
const mic = {
    ...
    headphoneOffsetWiredMs: 0,        // 有線イヤホン音ズレの画面補正(ms)。v0.9.79で目安を0ms（補正なし）に変更
    headphoneOffsetBluetoothMs: 0,    // Bluetoothイヤホン音ズレの画面補正(ms)。v0.9.84で目安を0ms（補正なし）に変更（判定ズレはbluetoothMicOffsetMsで扱う）
    headphoneOutputOffsetMs: 0, // 選択中の種類の音ズレ補正(ms)。互換用。v0.9.51でもUI/保存のみ・STAGE判定/マイク判定には未反映
    bluetoothMicOffsetMs: 0, // Bluetoothイヤホン時のマイクの遅れ補正(ms)。v0.9.80で新設。Bluetooth時のみ判定時刻に加算（負＝早める）。未保存は0=補正なし
    ...
    // 補正値(offset)は型別フィールド(timingOffsetMs/wiredMicOffsetMs/bluetoothMicOffsetMs)で既に分離済みなのでここには入れない。
    profiles: null,
...
const els = {
    ...
    proSettingsResetWrap: $('pro-settings-reset-wrap'),
    proSettingsResetBtn: $('pro-settings-reset-btn'),
    ...
    resultsRecordingClickToggle: $('results-recording-click-toggle'),       // v0.10.32：クリックを重ねるON/OFF
    ...
    judgePresetSeg: $('judge-preset-seg'),
    judgePresetNote: $('judge-preset-note'),
    ...
    calRetestMicBtn: $('cal-retest-mic-btn'),
    ...
    testReco: $('test-reco'),
    btMicDiagnostic: $('bt-mic-diagnostic'),
    btMicDiagnosticBody: $('bt-mic-diagnostic-body'),
    recoThr: $('reco-thr'),
...
/* PROカスタムSTAGE編集画面のプリセット（v0.9.247）。
   練習STAGE保存（state.rhythmProCustomStages）とは別ストアで管理し、一覧に混入させない。
   1件＝{ id, name, settings(正規化済みカスタムSTAGE設定), balance(0〜100), createdAt, updatedAt }。 */
const RHYTHM_CUSTOM_PRESETS_KEY = 'rhythmCruiseCustomPresets:v1';
const RHYTHM_CUSTOM_PRESET_LIMIT = 24;
const RHYTHM_CUSTOM_PRESET_NAME_MAX = 40;
...
function rhythmCustomPracticeStage() {
    if (!eng || !eng.editId || eng.testSource === 'rhythmCreate') return null;
    return getSavedRhythmCustomStages().find((s) => s.id === eng.editId) || null;
}
...
function setRhythmEditorZoom(scale) {
    ...
    if (proCustomEditDraft) proCustomEditDraft.zoom = rhythmVexZoomPrefs.editor;
    saveRhythmVexZoomPrefs();
...
function applyRhythmCustomStagePreviewZoom(stage) {
    rhythmVexZoomPrefs.stage = clampRhythmVexZoom(stage && stage.zoom);
    syncRhythmStagePreviewZoomUI();
}
...
function resetCurrentRhythmCustomPracticePrefs() {
    const stage = rhythmCustomPracticeStage();
    if (!stage) return;
    stopPreviewRhythm();
    if (state.running) stop();
...
function normalizeRhythmCreateUserPreset(raw) {
    ...
    const now = Date.now();
    // v0.9.247: BPM / 小節 / 拡大 / クリック-ストロークも保存・復元する。
    // 旧プリセット（これらのキーが無い）は null とし、呼び出し時は現在値を維持する（互換維持）。
    const bpm = Number.isFinite(Number(raw.bpm)) ? clampRhythmCreateBpm(raw.bpm) : null;
    const bars = Number.isFinite(Number(raw.bars)) ? clampRhythmCreateBars(raw.bars) : null;
    const zoom = Number.isFinite(Number(raw.zoom)) ? clampRhythmVexZoom(raw.zoom) : null;
    const balance = Number.isFinite(Number(raw.balance)) ? clampRhythmBalanceSliderValue(raw.balance) : null;
    return {
...
function renderRhythmCreateSavedList() {
    ...
    stages.forEach((s) => {
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'stage-card pro-custom-home-stage';
        btn.dataset.id = s.id;
        btn.innerHTML = `<span class="stage-num pro-custom-home-badge"><small>PRO</small><b>★</b></span><span class="stage-body"><span class="stage-title">${escapeHtml(s.title || '作成リズム')}</span></span><span class="stage-chevron">›</span>`;
        btn.addEventListener('click', () => openRhythmProCustomEditorFromSaved(s.id));
        els.rcSavedList.appendChild(btn);
    });
}
...
function openRhythmCreateStage(stageNo) {
    const def = getRhythmCreateStageDef(stageNo);
...
function renderRhythmCreateStage() {
...
function applyRhythmCreateUserPreset(preset) {
    ...
    // v0.9.247: 保存済みなら BPM / 拡大 / クリック-ストローク を先に復元する（旧プリセットは null＝現在値を維持）。
    if (preset.bpm != null) setRhythmCreateBpmFromValue(preset.bpm);
    if (preset.zoom != null) setRhythmCreateZoom(preset.zoom);
    if (preset.balance != null) applyRhythmBalanceSliderValue(els.rcCreatePreviewBalance, preset.balance);
    // 小節：保存値があれば優先。無ければ従来どおりパターン長以上を保証する。
    if (preset.bars != null) rhythmCreateBars = clampRhythmCreateBars(preset.bars);
    ...
    if (els.rcCreateBpm) els.rcCreateBpm.value = String(getRhythmCreateBpm());
    syncRhythmCreatePatternBarsUI();
...
function saveCurrentRhythmCreatePreset(initialName = '', modalTitle = 'プリセットに保存') {
    ...
    const preset = {
        ...
        // v0.9.247: 再生設定（BPM/小節/拡大/クリック-ストローク）も一緒に保存する。
...
function normalizeRhythmCustomStageSettings(raw) {
    ...
    return { version, id, title, description, grid, timeSignature, patternBars, bars, bpm, zoom, clickMode, rhythmFeel, pattern, motion, judgeTargets, displayLabels };
}
...
/* ═══════════ PROカスタムSTAGE編集画面のプリセット（v0.9.247）═══════════
   練習STAGE（state.rhythmProCustomStages）とは独立した localStorage ストア。
   編集中のパターン＋再生設定（BPM/小節/拡大/クリック-ストローク）に名前を付けて保存し、
   タップで編集画面へ読み込む。一覧表示（STAGE一覧）には混ざらない。 */
function makeRhythmCustomPresetId() {
    return 'rccustpreset_' + Date.now() + '_' + Math.random().toString(36).slice(2, 8);
}

function sanitizeRhythmCustomPresetName(name) {
    return String(name == null ? '' : name).trim().slice(0, RHYTHM_CUSTOM_PRESET_NAME_MAX);
}

function normalizeRhythmCustomPreset(raw) {
    if (!raw || typeof raw !== 'object') return null;
    const name = sanitizeRhythmCustomPresetName(raw.name);
    if (!name) return null;
    const settings = normalizeRhythmCustomStageSettings(raw.settings);
    if (!settings) return null;
    const now = Date.now();
    return {
        id: String(raw.id || makeRhythmCustomPresetId()),
        name,
        settings,
        balance: Number.isFinite(Number(raw.balance)) ? clampRhythmBalanceSliderValue(raw.balance) : 50,
        createdAt: Number.isFinite(Number(raw.createdAt)) ? Number(raw.createdAt) : now,
        updatedAt: Number.isFinite(Number(raw.updatedAt)) ? Number(raw.updatedAt) : now,
    };
}

function loadRhythmCustomPresets() {
    try {
        const raw = JSON.parse(localStorage.getItem(RHYTHM_CUSTOM_PRESETS_KEY) || '[]');
        if (!Array.isArray(raw)) return [];
        return raw.map(normalizeRhythmCustomPreset).filter(Boolean).slice(0, RHYTHM_CUSTOM_PRESET_LIMIT);
    } catch (e) {
        return [];
    }
}

function saveRhythmCustomPresets(list) {
    try {
...
function makeUniqueRhythmCustomPresetName(baseName, ignoreId = null) {
    const base = sanitizeRhythmCustomPresetName(baseName) || 'プリセット';
    const existing = new Set(loadRhythmCustomPresets().filter((p) => p.id !== ignoreId).map((p) => p.name));
    if (!existing.has(base)) return base;
    for (let i = 2; i < 1000; i++) {
        const cand = sanitizeRhythmCustomPresetName(base + ' (' + i + ')');
        if (!existing.has(cand)) return cand;
    }
    return sanitizeRhythmCustomPresetName(base + ' ' + Date.now());
}

/* 1件追加。上限超過なら null。成功したら正規化済みプリセットを返す。 */
function addRhythmCustomPreset(name, settings, balance) {
    const list = loadRhythmCustomPresets();
    if (list.length >= RHYTHM_CUSTOM_PRESET_LIMIT) return null;
    const preset = normalizeRhythmCustomPreset({
        id: makeRhythmCustomPresetId(),
        name: makeUniqueRhythmCustomPresetName(name),
        settings,
        balance,
    });
...
/* id指定で削除。削除できたら true。 */
function deleteRhythmCustomPresetById(id) {
    if (!id) return false;
    const list = loadRhythmCustomPresets();
...
function applyRhythmCustomEditorPreset(id) {
    ...
    renderRhythmCustomEditor();
    if (isStandardEdition()) applyStandardProCustomEditorSettingsLock();
...
function collectRhythmCustomEditorInputs() {
    ...
    d.zoom = clampRhythmVexZoom(rhythmVexZoomPrefs.editor);
...
function goTop() {
    ...
    stopPreviewRhythm(); // リズム作成/カスタム編集のプレビュー再生音を残さない。
...
/* ── テスト中断ガード（v0.9.91 → v0.9.92で拡張）─────────────────────────────
   いずれかのテスト（マイク反応 / 通常マイクの遅れ補正 / イヤホン音ズレの画面補正 /
...
function micDebugText() {
    ...
    return 'debugMic: ' + (MIC_DEBUG_ON ? 'on' : 'off') + '\n'
        ...
        + 'reviewPlayback.paused: ' + (rec ? (rec.playbackPaused ? 'yes' : 'no') : '-') + '\n'
        + 'reviewPlayback.pausePositionSec: ' + (rec && rec.playbackPaused && Number.isFinite(rec.playbackPausePositionSec) ? rec.playbackPausePositionSec.toFixed(3) + 's' : '-') + '\n'
        ...
        + 'reviewTimeline.paused: ' + (reviewTimelineDebug.paused ? 'yes' : 'no') + '\n'
...
/* データは結果カードと同じ state.micRunWave（t = gameAudioMs() + micJudgeOffsetMs() ＝判定時間軸）。
   ...
   イヤホン音ズレの画面補正 headphoneOutputOffsetMs）を表示位置に足して heard-time へ寄せる。
...
function drawReviewAdoptedTimeLines(ctx, w, h, yc, beatX, beatPx) {
    ...
    const msTextY = yc + 52;                         // drawReview() のズレ量テキストと同じ基準
    const lineTop = Math.max(top, h * 0.18);         // 小節番号より下、波形上部から開始
    const lineBot = Math.min(bot, msTextY - 16);     // 数字の文字高ぶん余白を残して止める
    ...
    if (MIC_DEBUG_ON && state.chordDebug && Array.isArray(state.chordDebug.onsets)) {
        ...
        for (const o of state.chordDebug.onsets) {
            ...
            ctx.beginPath(); ctx.moveTo(rx, lineTop); ctx.lineTo(rx, lineBot); ctx.stroke();
        }
        ctx.setLineDash([]);
    }
    ...
    for (let i = 0; i < TOTAL_BEATS; i++) {
        ...
        // 上端の採用マーカー（▼）も波形帯の内側に収め、小節番号や上端の文字と重ねない。
        ctx.globalAlpha = 1; ctx.fillStyle = col;
        ctx.beginPath(); ctx.moveTo(lx - 3, lineTop + 1); ctx.lineTo(lx + 3, lineTop + 1); ctx.lineTo(lx, lineTop + 6); ctx.closePath(); ctx.fill();
    }
    ctx.globalAlpha = 1;
...
function classify(diff) {
    ...
    const w = judgeWindows(); // v0.9.164：判定のきびしさプリセット。
    if (a <= w.justMs) return 'just';
    // EARLY/LATE 窓は拍間隔に応じて広げる（最低 w.nearMs、最大 拍間隔×w.nearFrac）。
    // 反応ラインを超えた入力は、拍の中間付近に来ない限り なるべく EARLY/LATE にする。
    const nearWin = Math.max(w.nearMs, state.beatInterval * w.nearFrac);
...
function registerHit(perfNow, source, direction) {
    ...
    if (occupiedCloser) {
        ...
        const w = judgeWindows(); // v0.9.164：分類と同じプリセット窓で「隣の空き拍へ寄せる」可否を判断
...
/* ── PROカスタムSTAGEサンプルシード（v0.9.206）────────────────
   三連符 / 8分のシャッフル / 16分のシャッフル を初回のみ
   rhythmProCustomStages の先頭に追加する。
   削除後は復活しない（フラグ管理）。 */
const RHYTHM_SAMPLE_STAGES_SEEDED_KEY = 'rhythmProCustomStageSamplesSeeded';

function seedRhythmCustomStageSamples() {
    if (localStorage.getItem(RHYTHM_SAMPLE_STAGES_SEEDED_KEY)) return;
    ...
    const samples = [
        ...
        },
        {
            ...
            rhythmFeel: 'swing', pattern: makeAlt(8),
        },
        {
            id: 'rc_sample_shuffle16', version: 1, title: '16分のシャッフル', description: '',
            grid: 'sixteenth', timeSignature: '4/4', patternBars: 1, bars: 4, bpm: 70,
            rhythmFeel: 'swing', pattern: makeAlt(16),
        },
    ];
    const existing = getSavedRhythmCustomStages();
    ...
    const toAdd = samples
        ...
        .filter(Boolean);
    if (toAdd.length > 0) {
        state.rhythmProCustomStages = [...toAdd, ...existing];
        saveSettings();
    }
...
/* コードモードPractice本番の実効クールダウン。
   保存値は反応テスト由来の余韻対策として尊重しつつ、8分/16分/スウィング等では
   その地点の実ターゲット間隔×45%を上限にして、次の正規入力まで塞がない。 */
function engChordEffectiveCooldownMs(hitTime) {
    const localGapMs = engChordLocalTargetGapMs(hitTime);
    if (localGapMs == null) return state.chordMinCooldown;
...
function engChordTargetGapSummary() {
    const targets = [];
    for (let i = 0; i < TOTAL_BEATS; i++) if (engIsHit(i)) targets.push({ beat: i, timeMs: engTargetTimeMs(i) });
    let minGapMs = Infinity, minGapTargetTimeMs = null, fastTargetCount = 0;
    for (let i = 0; i < targets.length; i++) {
        let localGapMs = Infinity;
        if (i > 0) localGapMs = Math.min(localGapMs, targets[i].timeMs - targets[i - 1].timeMs);
        if (i + 1 < targets.length) localGapMs = Math.min(localGapMs, targets[i + 1].timeMs - targets[i].timeMs);
        if (!Number.isFinite(localGapMs) || localGapMs <= 0) continue;
        if (localGapMs < minGapMs) { minGapMs = localGapMs; minGapTargetTimeMs = targets[i].timeMs; }
        if (localGapMs < CHORD_FAST_GAP_WARN_MS) fastTargetCount++;
    }
    return {
        minGapMs: Number.isFinite(minGapMs) ? minGapMs : null,
        minGapTargetTimeMs,
        fastTargetCount,
    };
}
...
function setPracticeRecordingPlaybackUI(playing) {
    const rec = state.practiceRecording;
    rec.playbackActive = !!playing;
    if (playing) rec.playbackPaused = false;
...
function getPracticeRecordingPlaybackPosition() {
    const rec = state.practiceRecording;
    if (rec.playbackPaused) {
        return Math.max(0, Number(rec.playbackPausePositionSec) || Number(rec.playbackOffsetSec) || 0);
    }
...
function playPracticeRecordingFromBuffer(offsetSec) {
    ...
    rec.playbackPaused = false;
    rec.playbackPausePositionSec = 0;
    let source;
...
function pausePracticeRecordingPlayback() {
    const rec = state.practiceRecording;
    if (!rec.playbackActive || rec.status !== 'available') return;
    const pos = getPracticeRecordingPlaybackPosition();
    rec.playbackPausePositionSec = pos;
    rec.playbackOffsetSec = pos;
    rec.playbackPaused = true;
    stopReviewTimelineRaf(false);
    stopPracticeRecordingBufferSource();
    stopReviewClickOverlay();
    rec.playbackActive = false;
    updatePracticeRecordingPlaybackButtons();
    updateReviewTimelineUI(pos);
    updatePracticeRecordingDebugUI();
}

function resumePracticeRecordingPlayback() {
    const rec = state.practiceRecording;
    if (!rec.playbackPaused || rec.status !== 'available') return false;
    const pos = Math.max(0, Number(rec.playbackPausePositionSec) || Number(rec.playbackOffsetSec) || 0);
    return playPracticeRecordingFromBuffer(pos);
}
...
function scheduleReviewClick(audioTime, accent) {
    ...
    if (!ctx) return;
    const vol = Math.max(0, Math.min(1, rec.reviewClickVolume / 100));
    if (vol <= 0) return; // 0%は実質無音
    const peak = Math.min(CLICK_PEAK_CLIP, clickBasePeak(accent) * vol * clickPeakGain());
    if (peak < 0.001) return;
    const t0 = Math.max(audioTime, ctx.currentTime); // 過去時刻なら即時
    const osc = ctx.createOscillator();
    const gain = ctx.createGain();
    const stopAt = applyClickEnvelope(ctx, osc, gain, t0, accent, peak); // Practice本番と同じ音色・エンベロープを再利用
    osc.connect(gain).connect(ctx.destination);
    osc.start(t0);
    osc.stop(stopAt);
...
function cleanupReviewClickOverlay() {
    stopReviewClickOverlay();
    const rec = state.practiceRecording;
...
function applyReviewClickUI() {
    const rec = state.practiceRecording;
    if (els.resultsRecordingClickToggle) els.resultsRecordingClickToggle.checked = !!rec.reviewClickEnabled;
...
function startPracticeRecording() {
    ...
    // v0.10.32：録音開始(=game-time 0)基準の相対秒で、Practice時のクリック予定（カウントイン込み）をコピー。
    //   判定用のタイミング補正/Bluetooth補正は混ぜない。録音再生のクリック重ね専用の一時データ。
...
function chordMissAnalyze() {
    ...
    for (let i = 0; i < TOTAL_BEATS; i++) {
        ...
        if (r && r.tapped) {
            ...
            continue; // 登録済み（タイミング外を含む）は「立ち上がり未検出」ではない
        }
        ...
        rows.push({
            ...
            ピークズレms: (pb.offsetMs != null ? pb.offsetMs : ''), クールダウン中: !!pb.cooldownBlocked,
            棄却時ズレms: (pb.cooldownBlockedAtMs != null ? pb.cooldownBlockedAtMs : ''),
            前回採用からms: (pb.sinceHitMs != null ? pb.sinceHitMs : ''),
    ...
    return { count: rows.length, timingMissCount, reasons, lines };
}

/* Phase2 ログ可視化用の入れ物（?debugMic=1 のときだけ resetData で確保）。
...
function updateChordDev() {
    ...
    const hideAll = () => {
        [els.resultsChordDev, els.resultsChordSpeedWarning, els.resultsChordDetail, els.resultsChordMiss, els.resultsChordDebug].forEach((e) => { if (e) e.classList.add('hidden'); });
    };
    ...
    // ④ Phase2 ログ可視化サマリー（?debugMic=1 のときだけ・Lateの内訳を見える化）。
    if (els.resultsChordDebug) {
        const cd = state.chordDebug;
        if (!MIC_DEBUG_ON || !cd) { els.resultsChordDebug.classList.add('hidden'); }
        else {
            ...
            const effectiveCdMed = med2('effectiveCooldownMs');
            const minGapMs = gapSummary.minGapMs != null ? Math.round(gapSummary.minGapMs) : null;
            const representativeEffectiveCd = gapSummary.minGapTargetTimeMs != null
                ? Math.round(engChordEffectiveCooldownMs(gapSummary.minGapTargetTimeMs)) : null;
            const jw = judgeWindows();
            const judgeMaxMs = Math.round(Math.max(jw.nearMs, state.beatInterval * jw.nearFrac));
            const worst = cd.onsets.filter((o) => typeof o.adoptedDiff === 'number').slice().sort((a, b) => Math.abs(b.adoptedDiff) - Math.abs(a.adoptedDiff)).slice(0, 3);
            const lines = worst.map((o, i) => (i + 1) + '. 拍' + (o.beat != null ? (o.beat + 1) : '--')
                + ' ' + (o.cls === 'miss' ? 'タイミング外' : (o.cls || '--')) + ' / target ' + fmtT(o.targetMs)
            ...
            els.resultsChordDebug.textContent = '🛠（debugMic）コード判定デバッグ｜検出数 ' + cd.onsets.length
                ...
                + ' ｜ バックデート中央値 ' + bdMed + 'ms'
                + ' ／ 適応上限中央値 ' + (capMed != null ? capMed + 'ms' : '--')
                + ' ／ localGap 中央値 ' + (gapMed != null ? gapMed + 'ms' : '--') + '・最小 ' + (minGapMs != null ? minGapMs + 'ms' : '--')
                + ' ／ 保存クールダウン（余韻対策）' + state.chordMinCooldown + 'ms'
                + ' ／ 実効クールダウン（譜面上限後）' + (effectiveCdMed != null ? effectiveCdMed + 'ms' : (representativeEffectiveCd != null ? representativeEffectiveCd + 'ms' : '--'))
                + ' ／ 判定幅 GOOD ±' + jw.justMs + 'ms・EARLY/LATE ±' + judgeMaxMs + 'ms'
...
function loadStageClickSettings() {
    try {
        ...
        return {
            range: STAGE_CLICK_RANGES.includes(raw.range) ? raw.range : STAGE_CLICK_DEFAULTS.range,
            beats: STAGE_CLICK_BEATS.includes(raw.beats) ? raw.beats : STAGE_CLICK_DEFAULTS.beats,
            offbeat: !!raw.offbeat,
        };
    } catch (_) { return Object.assign({}, STAGE_CLICK_DEFAULTS); }
}
...
/* v0.9.153：イヤホン専用処理のゲート。
   通常マイクでは、本体スピーカーのクリック音を本体マイクが拾うことが自然なので、
   クリック音漏れチェック/80%固定/イヤホン用導線は絶対に通さない。 */
function shouldUseEarphoneMicTestFeatures() {
    return getMicInputType() === 'headphone' && (getHeadphoneType() === 'wired' || getHeadphoneType() === 'bluetooth');
}
...
function updateMicInputTypeUI() {
    ...
    // 「イヤホン音ズレの画面補正」カードの表示可否は種類（Bluetoothのみ）次第なので updateHeadphoneTypeUI() に委ねる。
...
/* イヤホン音ズレの画面補正テスト専用BPM（v0.9.92→v0.9.93で範囲変更）。STAGE/最終確認テストのテンポには影響しない一時設定。 */
const HP_CAL_BPM_MIN = 20, HP_CAL_BPM_MAX = 160, HP_CAL_BPM_STEP = 20;
let hpCalBpm = 80;                 // イヤホン音ズレの画面補正テストのデフォルトBPM（v0.9.100で100→80へ）
...
/* Bluetoothイヤホンの「標準」スタートライン(ms)。判定ズレ補正ではなく、イヤホン音と画面表示を
   ...
   ・マイク設定側(MIC=100ms)：マイクの遅れ補正画面の表示/クリック合わせ用。
...
let hpLane = { ctx: null, w: 0, h: 0 }; // イヤホン音ズレの画面補正の流れるレーン（v0.9.91）
...
const HP_MANUAL_NOTE = {
    wired: 'イヤホンで音が遅れて聞こえる分だけ画面表示を合わせます。判定時間は変わりません。通常は0msで問題ありません。',
    bluetooth: 'イヤホンで音が遅れて聞こえる分だけ画面表示を合わせます。判定時間は変わりません。Bluetoothでは大きくなることがあります。',
};
function getHeadphoneType() { return mic.headphoneType === 'bluetooth' ? 'bluetooth' : 'wired'; }
/* Bluetoothイヤホン接続中か（マイクの遅れ補正の表示/判定反映の条件・v0.9.80） */
...
/* 判定時刻に使うマイク補正（ms）。通常/有線は timingOffsetMs のみ。
   ...
   イヤホン音ズレの画面補正 headphoneOutputOffsetMs はここに含めない（判定には反映しない）。 */
function micJudgeOffsetMs() {
    // 現在の入力タイプに応じて、その種類の補正値を返す（v0.9.153）。
    // 通常マイク＝timingOffsetMs のみ。イヤホンは timingOffsetMs を土台に、有線は wiredMicOffsetMs、
    // BluetoothはbluetoothMicOffsetMs を加算する（有線/BTの手拍子補正は通常マイクのtimingOffsetMsを書き換えない）。
    if (isBluetoothHeadphone()) return mic.timingOffsetMs + (mic.bluetoothMicOffsetMs || 0);
...
/* イヤホン音ズレの画面補正レーン（v0.9.91）：STAGE/最終確認テスト風に、右→左へ4拍の丸が流れる。
...
/* イヤホン音ズレの画面補正テスト専用BPMの変更（v0.9.93→v0.9.94）。
...
/* 種類選択UI・説明文・カード表示・スライダー表示を、選択中の種類に合わせて更新。
   v0.9.52：有線は「イヤホン音ズレの画面補正」カードを隠し（手動設定で調整）、Bluetoothのみカード表示。 */
function updateHeadphoneTypeUI() {
    ...
    // 「イヤホン音ズレの画面補正」カードは headphone かつ Bluetooth のときだけ表示
...
/* デバッグ用（v0.9.105）：マイクの遅れ補正の判定値を確認したいときだけ true。本番は false。 */
...
function headphoneMicOffsetSet(v) {
    ...
    // 有線は wiredMicOffsetMs に保存（通常マイクの timingOffsetMs を汚染しない・v0.9.153）。
...
function ptAssignOnset(tMs, peak) {
    ...
    if (occupiedCloser) {
        ...
        if (cand >= 0 && cand <= PT_PLAY_BEATS - 1 && pt.notes[cand].cls == null && Math.abs(diffTo(cand)) <= ptNearWin()) {
            i = cand;
            diff = diffTo(i);
    ...
    if (!prev || Math.abs(diff) < Math.abs(note.diff)) {
        ...
        const sign = diff > 0 ? '+' : (diff < 0 ? '−' : '±');
        setPtStatus((LABELS[note.cls] || '') + ' ' + sign + Math.abs(Math.round(diff)) + 'ms');
...
function drawPracticeLane(t) {
    ...
    // v0.9.106：イヤホン音ズレの画面補正（headphoneOutputOffsetMs＝Bluetooth時はheadphoneOffsetBluetoothMs）を
    ...
    for (let i = 0; i < pt.notes.length; i++) {
        ...
        ctx.restore();
        const inWin = (n.cls == null) && Math.abs(t - n.t) <= ptNearWin();
        let col;
        ...
        // 最終確認テスト専用のライブ判定表示。既に確定した cls/diff を描くだけで、判定処理には関与しない。
        if (n.cls && n.diff != null) {
            const mx = x + n.diff * ppm;
            const markerY = yc - 38;
            const label = n.cls === 'just' ? 'GOOD' : (n.cls === 'early' ? 'EARLY' : 'LATE');
            const sign = n.diff > 0 ? '+' : (n.diff < 0 ? '−' : '±');
            ctx.save();
            ctx.strokeStyle = col;
            ctx.lineWidth = 1;
            ctx.setLineDash([3, 3]);
            ctx.beginPath(); ctx.moveTo(x, markerY); ctx.lineTo(mx, markerY); ctx.stroke();
            ctx.setLineDash([]);
            ctx.fillStyle = col;
            ctx.beginPath(); ctx.arc(mx, markerY, 4, 0, Math.PI * 2); ctx.fill();
            ctx.font = '800 10px Outfit, sans-serif';
            ctx.textAlign = 'center';
            ctx.fillText(label, mx, markerY - 9);
            ctx.font = '700 9px Outfit, sans-serif';
            ctx.fillText(sign + Math.abs(Math.round(n.diff)) + 'ms', mx, markerY + 15);
            ctx.restore();
        }
    }
...
function ptLoop() {
    ...
    for (let i = 0; i < pt.notes.length; i++) {
        const n = pt.notes[i];
        if (!n.closed && t > n.t + ptNearWin()) n.closed = true;
    }
    drawPracticeLane(t);
...
function practiceComment(r) {
    ...
    // イヤホン接続（有線/Bluetooth）は手拍子マイクの遅れ補正テストへ誘導（v0.9.152で有線も対応）。
    const isHp = isHeadphoneInput();
    if (biased && (r.avg >= 40 || (lateRatio >= 0.7 && lateRatio >= earlyRatio))) {
        if (isHp) return { kind: 'warn', issue: 'btdelay', text: 'イヤホンではマイク判定が遅め（LATE）にずれている可能性があります。「マイクの遅れ補正」で手拍子に合わせて補正してから、もう一度最終確認テストを行ってください。' };
    ...
    if (biased && (r.avg <= -40 || (earlyRatio >= 0.7 && earlyRatio > lateRatio))) {
        if (isHp) return { kind: 'warn', issue: 'btdelay', text: 'イヤホンではマイク判定が早め（EARLY）にずれている可能性があります。「マイクの遅れ補正」で手拍子に合わせて補正してから、もう一度最終確認テストを行ってください。' };
...
function renderPracticeResult(r) {
    ...
    if (c.kind === 'ok') {
        ...
        actions =
            ...
            '<button type="button" id="pt-result-done" style="' + dark + '">保存せずにこの設定を使う</button>';
    } else {
...
function practiceFixGoTo(route) {
    ...
    if (route === 'test') {
        ...
        // Bluetooth用マイクの遅れ補正へ：そのステップを開く（v0.9.80）
...
/* 通常マイクの補正テスト失敗時、「マイク反応テストをやり直す」へ戻す導線（v0.9.153）。 */
...
/* マイクの遅れ補正バッジ（実施済み/未実施）を現在の進捗フラグに同期する（v0.9.163）。
...
function renderBtCalIdle() {
    if (els.btCalBtn) els.btCalBtn.textContent = bt.hasRun ? 'もう1度テストする' : 'マイクの遅れ補正テストを開始';
...
async function startBtCal() {
    ...
    if (els.btCalBtn) els.btCalBtn.textContent = 'マイクの遅れ補正テストを停止';
...
function renderBtCalResult(r) {
    ...
    const sub = 'width:100%;padding:12px;margin-top:8px;border-radius:10px;border:1px solid rgba(255,255,255,0.28);'
        + 'background:rgba(255,255,255,0.05);color:inherit;font-weight:700;cursor:pointer;';
    // 自動微調整を適用した場合は、最新値（appliedOffset）を「現在のマイクの遅れ補正」に表示する（v0.9.95）
...
/* v0.9.100：マイクの遅れ補正(bluetoothMicOffsetMs)を結果画面から手動微調整するスライダーブロック。
...
/* BPM変更：テスト中なら停止して待機へ戻す（イヤホン音ズレの画面補正と同様にBPMはテスト外で決める）。 */
...
function loadSettings() {
    ...
    {
        // イヤホン音ズレの画面補正（v0.9.51）：種類別に保存。無い/範囲外/不正値は各種類の目安に戻す。
    ...
    // 入力タイプ別マイク設定プロファイル（v0.9.153）。古いデータには無いので安全にフォールバックする。
    loadMicProfilesFromStored(s.micProfiles);
...
function applySettingsToUI() {
    ...
    updateStrokeDetectModeUI();
    updateJudgePresetUI(); // 判定のきびしさ（v0.9.164）：保存値どおりにセグメント点灯・補足文を反映
...
/* 設定タブ（v0.9.95）：'mic'（既存マイク設定フロー）/ 'tap'（画面タップ設定）。
   ...
   'home'（補正値＋3ボタン）/ 'cal'（イヤホン音ズレの画面補正カードを再利用）/ 'preset'（タップ用プリセット）/ 'manual'（手動設定）。 */
let settingsTab = 'mic';
let tapView = 'home';
...
function wizardStepCards(id) {
    switch (id) {
        ...
        case 'correction':
            // 通常マイク＝マイクの遅れ補正／Bluetooth＝イヤホン音ズレの画面補正（有線はこのステップ自体が無い）
...
function renderSettingsView() {
    ...
    if (els.proSettingsResetWrap) els.proSettingsResetWrap.classList.add('hidden');
...
const JUDGE_PRESET_LABELS = { easy: '易しい', standard: 'やや易しい', semiStrict: '標準', strict: 'やや厳しい', veryStrict: '厳しい' };
function judgePresetNoteText(key) {
    const w = JUDGE_PRESETS[key] || JUDGE_PRESETS.standard;
    return JUDGE_PRESET_LABELS[key] + '：GOOD ±' + w.justMs + 'ms ／ EARLY・LATE ±' + w.nearMs
        + 'ms〜（速いテンポでは拍間隔の' + Math.round(w.nearFrac * 100) + '%まで広がります）';
}
function updateJudgePresetUI() {
    const cur = normalizeJudgePreset(state.judgePreset);
    if (els.judgePresetSeg) {
        els.judgePresetSeg.querySelectorAll('[data-judge]').forEach((b) => {
            const active = b.getAttribute('data-judge') === cur;
            b.classList.toggle('is-active', active);
            b.setAttribute('aria-pressed', active ? 'true' : 'false');
        });
    }
    if (els.judgePresetNote) els.judgePresetNote.textContent = judgePresetNoteText(cur);
}
/* セグメントタップで判定プリセットを切り替えて保存（通常STAGE/カスタム/くり返し/最終確認テスト共通）。 */
function setJudgePreset(key) {
    const next = normalizeJudgePreset(key);
    if (state.judgePreset !== next) {
        state.judgePreset = next;
        saveSettings();
    }
    updateJudgePresetUI();
}
...
const WIZARD_STEP_LABELS = {
    ...
    correction: '補正', btdelay: 'マイクの遅れ補正', practice: '最終確認テスト', final: '手動設定・保存',
};
...
/* テスト実施履歴（UIの「もう1度／もう一度テストする」表示）だけをリセットして、
   ...
   ・最終確認テスト：最終確認テストを開始
   ・Bluetooth用マイクの遅れ補正：マイクの遅れ補正テストを開始
   ・マイク反応テスト：テストを開始
   ・マイクの遅れ補正：テストを開始
   ※イヤホン音ズレの画面補正の待機文言は常に「補正テスト開始」で履歴を持たない。 */
...
function resetSetupFlowDisplay() {
    ...
    bt.hasRun = false; // マイクの遅れ補正の実施履歴もリセット
    stopBtCal();
    saveSettings();
    // 表示クリア
...
/* ── 入力タイプ別マイク設定プロファイル（v0.9.153）────────────────────────────
   clickVolume / threshold / lowInputProfile を「通常マイク/有線/Bluetooth」で分離保存・復元する。
   目的：イヤホン用追加処理（80%固定・低入力プロファイル等）が、通常マイクの設定を汚染しないようにする。
   補正値(offset)は型別フィールドで既に分離済みなので、このプロファイルには含めない。 */
function micProfileKeyFor(type, hpType) {
    const t = type || getMicInputType();
    if (t !== 'headphone') return 'normal';
    const h = hpType || getHeadphoneType();
    return (h === 'bluetooth') ? 'bluetooth' : 'wired';
}
function currentMicProfileKey() { return micProfileKeyFor(); }
/* 現在アクティブな共有設定を、指定キーのプロファイルへ退避する。 */
function storeActiveMicProfile(key) {
    if (!key) return;
    if (!mic.profiles || typeof mic.profiles !== 'object') mic.profiles = {};
    mic.profiles[key] = {
        clickVolume: state.clickVolume,
        threshold: mic.threshold,
        lowInputProfile: !!mic.lowInputProfile,
...
/* 指定キーのプロファイルをアクティブな共有設定へ復元する。プロファイルが無ければ false。 */
function restoreMicProfile(key) {
    const p = mic.profiles && mic.profiles[key];
    if (!p || typeof p !== 'object') return false;
    state.clickVolume = clampNum(p.clickVolume, 0, 100, state.clickVolume);
    mic.threshold = clampNum(p.threshold, THR_MIN, THR_MAX, mic.threshold);
    mic.lowInputProfile = !!p.lowInputProfile;
    return true;
}
/* 通常マイク用プロファイルが無い場合の初期値（既定）。イヤホン側の値を持ち込まない。 */
function initNormalMicProfileDefault() {
    state.clickVolume = SETTINGS_DEFAULTS.clickVolume;
    mic.threshold = SETTINGS_DEFAULTS.threshold;
    mic.lowInputProfile = false;
}
/* 保存済みプロファイルを読み込んで mic.profiles を構築する（v0.9.153・loadSettingsから呼ぶ）。
   ・古いlocalStorageには micProfiles が無いので安全にフォールバックする。
   ・現在の入力タイプは、フラットな保存値（threshold/clickVolume/lowInputProfile）が最新なので常にそれで同期する。
   ・通常マイク用プロファイルは常に用意し、無ければ既定値で作る（イヤホン側の80%固定等を持ち込まない）。 */
function loadMicProfilesFromStored(stored) {
    const out = {};
    const sanitize = (p) => {
        if (!p || typeof p !== 'object') return null;
        return {
            clickVolume: clampNum(p.clickVolume, 0, 100, SETTINGS_DEFAULTS.clickVolume),
            threshold: clampNum(p.threshold, THR_MIN, THR_MAX, SETTINGS_DEFAULTS.threshold),
            lowInputProfile: !!p.lowInputProfile,
        };
    };
    if (stored && typeof stored === 'object') {
        ['normal', 'wired', 'bluetooth'].forEach((k) => {
            const v = sanitize(stored[k]);
            if (v) out[k] = v;
        });
    }
    const curKey = currentMicProfileKey();
    out[curKey] = { clickVolume: state.clickVolume, threshold: mic.threshold, lowInputProfile: !!mic.lowInputProfile };
    if (!out.normal) {
        out.normal = { clickVolume: SETTINGS_DEFAULTS.clickVolume, threshold: SETTINGS_DEFAULTS.threshold, lowInputProfile: false };
    }
    mic.profiles = out;
}

/* 入力タイプ/イヤホン種類を prevKey から切り替えたあと、新しいキーの設定を復元する（v0.9.153）。
   切替前に storeActiveMicProfile(prevKey) を呼んでおくこと。 */
function restoreMicProfileForCurrent(prevKey) {
    const newKey = currentMicProfileKey();
    if (newKey === prevKey) return; // 種類が変わっていなければ何もしない
    if (!restoreMicProfile(newKey)) {
        // 未作成：通常は既定、イヤホンは50%初期化（既存のイヤホン初期値）。
        if (newKey === 'normal') initNormalMicProfileDefault();
        else applyEarphoneClickVolumeDefault();
    }
    applySettingsToUI();
    saveSettings();
}
...
function onPickInputType(type) {
    ...
    // イヤホン用の50%/80%や低入力プロファイルが通常マイクへ持ち込まれない（その逆も）。
...
const MIC_PRESET_COMPARE_KEYS = ['inputType', 'headphoneType', 'strokeDetectMode', 'threshold', 'cooldownMs',
    'clickVolume', 'timingOffsetMs', 'bluetoothMicOffsetMs', 'wiredMicOffsetMs', 'lowInputProfile',
...
function restoreMicSettingsFrom(s) {
    ...
    if (typeof s.wiredMicOffsetMs === 'number') mic.wiredMicOffsetMs = s.wiredMicOffsetMs;
    mic.lowInputProfile = !!s.lowInputProfile;
...
function applyPresetCore(id) {
    ...
    // Bluetooth用マイクの遅れ補正：未保存（古いプリセット）は0=補正なし（v0.9.80）
    mic.bluetoothMicOffsetMs = clampNum(s.bluetoothMicOffsetMs, BT_MIC_OFFSET_MIN, BT_MIC_OFFSET_MAX, 0);
    ...
    // 適用後の値を、その入力タイプのプロファイルへ同期しておく（v0.9.153）。
    ...
    setupProgress.btDelayDone = true;    // マイクの遅れ補正値も呼び出し済み扱い（v0.9.80）
...
function resetSettings() {
    ...
    mic.wiredMicOffsetMs = 0;
    mic.profiles = null; // 入力タイプ別プロファイルも初期化（v0.9.153。次のload/切替で作り直す）
...
function resetBtCalTransientUiState() {
    ...
    updateBtCalBadge();
    if (els.btCalBtn && !bt.active) els.btCalBtn.textContent = 'マイクの遅れ補正テストを開始';
}
...
function resetMicReactionTestRunDisplay() {
    ...
    if (els.btMicDiagnostic) els.btMicDiagnostic.classList.add('hidden');
...
const BT_STRONG_SHORT_MAX_RECO_SENS = 65; // 入力十分・検出少なめ時は過剰な高感度化を避ける
...
function beginStrokePhase() {
    ...
    for (let i = 0; i < COUNT; i++) {
        ...
        test.strokePeriodBeatPeaks.push(0);
    }
    test.noteIdx = 0;
...
function renderBtMicDiagnostic() {
    if (!els.btMicDiagnostic || !els.btMicDiagnosticBody) return;
    if (!isBluetoothHeadphone() || !test.btDiag) {
        els.btMicDiagnostic.classList.add('hidden');
        els.btMicDiagnosticBody.innerHTML = '';
        return;
    }
    const d = test.btDiag;
    const row = (k, v) => '<div class="bt-diag-row"><span>' + k + '</span><b>' + v + '</b></div>';
    const beatRows = (d.beats || []).map((b) =>
        row(b.n + '回目', (b.detected ? '検出あり' : '検出なし')
            + ' / 窓 ' + btDiagNumber(b.windowPeak)
            + ' / 期間 ' + btDiagNumber(b.periodPeak))
    ).join('');
    els.btMicDiagnosticBody.innerHTML =
        '<div class="bt-diag-grid">' +
        row('入力タイプ', 'Bluetoothイヤホン') +
        row('検出数', d.detected + ' / ' + d.total) +
        row('推奨マイク感度', d.recoSens === '–' ? '–' : d.recoSens + '%') +
        ...
        row('検出窓ピーク平均', btDiagNumber(d.windowAvg)) +
        row('検出窓ピーク最大', btDiagNumber(d.windowMax)) +
        row('クリック推定ピーク', btDiagNumber(d.projectedClickPeak)) +
        row('ノイズ基準', btDiagNumber(d.noiseLine)) +
        row('入力判定', d.inputEnough ? '十分' : '不足') +
        row('検出判定', d.detectEnough ? '十分' : '少なめ') +
        row('クリック拾い警告', d.clickLeak ? 'あり' : 'なし') +
        row('BT補助ピーク使用', d.btPeriodUsed ? 'あり' : 'なし') +
        row('推奨ライン抑制', d.recoCapped ? 'あり' : 'なし') +
        '</div>' +
        '<div class="bt-diag-beats">' + beatRows + '</div>';
    els.btMicDiagnostic.classList.remove('hidden');
}
...
function updateReco() {
    ...
    const btPeriodUsed = isBluetoothHeadphone() && btPeriodP75 != null && (maxStrokeRaw == null || btPeriodP75 >= maxStrokeRaw);
    ...
    if (test.recommended != null) {
        ...
        if (btStrongButStrokeShort && recoSensDisplay(test.recommended, lowInputTuned) > BT_STRONG_SHORT_MAX_RECO_SENS) {
            test.recommended = thresholdFromSens(BT_STRONG_SHORT_MAX_RECO_SENS);
            btRecoSensCapped = true;
            highSens = false;
            els.recoThr.textContent = (provisional ? '仮 ' : '') + recoSensDisplay(test.recommended, false) + '％';
        }
    }
    ...
    if (isBluetoothHeadphone()) {
        ...
        const winAvg = sp.length ? sp.reduce((a, b) => a + b, 0) / sp.length : null;
        const beats = (test.notes || []).map((n, i) => ({
            n: i + 1,
            detected: !!n.detected,
            windowPeak: n.peak,
            periodPeak: test.strokePeriodBeatPeaks ? test.strokePeriodBeatPeaks[i] : null,
        }));
        test.btDiag = {
            detected: detectedStrokeNotes,
            total: totalStrokeNotes,
            recoSens: test.recommended != null ? recoSensDisplay(test.recommended, lowInputTuned) : '–',
            ...
            windowAvg: winAvg,
            windowMax: strokeSamples.length ? strokeSamples[strokeSamples.length - 1] : null,
            projectedClickPeak: test.projClickMax,
            noiseLine: lowInputNoiseLine,
            inputEnough: !!btInputLooksStrong,
            detectEnough: !strokeDetectionShort,
            clickLeak: !!test.earphoneClickLeak,
            btPeriodUsed,
            recoCapped: btRecoSensCapped,
            beats,
        };
    } else {
        test.btDiag = null;
...
function micLoop() {
    ...
    // ── マイクの遅れ補正（v0.9.80・Bluetooth用）：クリックに対する入力時刻だけを集める（registerHit/スコアには流さない）──
    if (bt.active) {
        if (els.btCalLevel) {
    ...
    if (pt.active) {
        ...
        if (pt.capturing) {
            ...
            for (let bi = 0; bi < PT_PLAY_BEATS; bi++) {
                ...
                if (Math.abs(bd) <= ptNearWin() && peak > pt.notes[bi].peak) pt.notes[bi].peak = peak;
            }
    ...
    if (state.strokeDetectMode === 'chord' && state.running) {
        ...
        const chordHitTime = gameAudioMs() + micJudgeOffsetMs();
        const effectiveChordCooldownMs = engChordEffectiveCooldownMs(chordHitTime);
        ...
        if (!inCountIn && nearestBeat >= 0) {
            ...
            if (peak >= detThr) {
                ...
                if (riseFromValley >= state.chordRiseGate && instantRise >= state.chordInstantRiseGate && sinceHit < effectiveChordCooldownMs) {
                    pb.cooldownBlocked = true;
                    pb.cooldownBlockedAtMs = Math.round(chordHitTime - engTargetTimeMs(nearestBeat));
                    pb.sinceHitMs = Math.round(sinceHit);
                    pb.effectiveCooldownMs = Math.round(effectiveChordCooldownMs);
                }
            }
        }
        ...
        if (MIC_DEBUG_ON && state.chordDebug && !inCountIn) {
            ...
            const instantRiseOk = instantRise >= state.chordInstantRiseGate;
            // アタックの起点（ライン到達 or 余韻中の再アタック）の時刻を記録
            if (thresholdOk && mic.prevPeak < detThr) {
                cd.epStartMs = gtw; cd.epBlockCooldown = 0; cd.epBlockRise = 0; cd.epBlockInstant = 0;
            } else if (thresholdOk && cd.epStartMs == null && instantRise > 0) {
                cd.epStartMs = gtw; cd.epBlockCooldown = 0; cd.epBlockRise = 0; cd.epBlockInstant = 0;
            }
            // 採用までの遅延要因（ライン超え中・未採用フレーム）を累積
            if (thresholdOk && !chordOnset) {
                if (!cooldownOk && riseOk && instantRiseOk) cd.epBlockCooldown++;
                else if (cooldownOk && !riseOk) cd.epBlockRise++;
                else if (cooldownOk && riseOk && !instantRiseOk) cd.epBlockInstant++;
            }
            // ライン超え候補の棄却カウント（既存 chordIgnoredRePeaks と同じ「候補」条件で分類）
            if (thresholdOk && !chordOnset && instantRise >= state.chordInstantRiseGate * 0.5) {
                if (!cooldownOk) cd.rejCooldown++;
                if (!riseOk) cd.rejRise++;
                if (!instantRiseOk) cd.rejInstant++;
            }
            ...
            if (inCountIn) {
                ...
                if (ok) {
                    ...
                    if (MIC_DEBUG_ON && state.chordDebug) {
                        ...
                        const lineCrossDiff = (targetMs != null) ? (lineCrossMs - targetMs) : null;
                        const lineCrossStale = (lineCrossDiff != null && localTargetGapMs != null)
                        ...
                        const onsetEntry = {
                            ...
                            effectiveCooldownMs: Math.round(effectiveChordCooldownMs),                                  // 実ターゲット間隔上限適用後
                            lineCrossDiff,                                                                               // ライン超え時刻のズレ（debug episode）
...
function bind() {
    ...
    if (els.rhythmCreateStageList) els.rhythmCreateStageList.addEventListener('click', (e) => {
        ...
        if (stage && stage.ready) openRhythmCreateStage(n);
    });
    ...
    document.querySelectorAll('.setting-note-btn').forEach((btn) => {
        btn.addEventListener('click', () => {
            ...
            const body = row ? row.querySelector('.setting-note-body') : btn.nextElementSibling;
            if (!body) return;
    ...
    // 反応ライン・二重反応防止・マイクの遅れ補正は実践テストの判定に効くので、
    ...
    // v0.10.32：クリックを重ねるON/OFF。再生中にOFFなら予約停止、ONなら現在位置から開始。
    if (els.resultsRecordingClickToggle) {
        els.resultsRecordingClickToggle.addEventListener('change', () => {
            const rec = state.practiceRecording;
    ...
    // v0.10.32：クリック音量（録音音量とは別の一時state）。先読み幅ぶんで反映される。
    if (els.resultsRecordingClickVolume) {
        els.resultsRecordingClickVolume.addEventListener('input', () => {
            const rec = state.practiceRecording;
            const n = Number(els.resultsRecordingClickVolume.value);
    ...
    if (els.proSettingsResetBtn) els.proSettingsResetBtn.addEventListener('click', resetProSettingsToDefaults);
    // 判定のきびしさ（v0.9.164）：易しい / やや易しい / 標準 / やや厳しい / 厳しい
    if (els.judgePresetSeg) els.judgePresetSeg.querySelectorAll('[data-judge]').forEach((b) => {
