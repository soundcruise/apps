...
- **結果カード**: v1.0.11で「ズレ履歴」グラフに小節番号の補助表示を追加。Canvas上の表示専用で、判定・集計・録音レビュー・保存ロジックには影響しない。
- **保存済みSTAGEの再編集**: v1.0.12で、保存済みSTAGEのPracticeから編集へ進む際、直前の別STAGEの編集ドラフトではなく現在のPractice対象IDを読み直すよう修正。編集画面からPracticeへ渡した同一IDの未保存ドラフトは従来どおり維持する。
- **クリックを鳴らす範囲**: v1.1.0で「1小節ごと」を追加。Practice本編は奇数小節だけクリックを鳴らし、偶数小節では表拍・裏拍・アクセントを鳴らさない。カウントイン、判定、録音、表示、保存済みSTAGE形式は従来どおり。
- **設定画面**（`screen-settings`）: タブ切り替えで以下4つ。
  ...
  - **クリック音**（`settings-tab-click`）: クリックを鳴らす範囲・拍・裏拍設定。v1.0.8で裏拍ON/OFFを既存の `#offbeat-toggle` のままスイッチ風UIにし、`renderClickDots()` は4/4概念の「1小節」「1拍目〜4拍目」「表/裏」拍マップ表示に変更した。v1.0.9で裏拍トグルを拍マップ直前へ移動し、小さめのスイッチに調整。クリック音ロジック・`rcClickBeats` / `rcClickOffbeat` の意味・localStorage形式は変更なし。
...
- PRO版の戻り導線・相対パス注意:
  ...
  - 各静的ページ（`info.html` 等）は `edition` パラメータや `document.referrer`、`sessionStorage`（キー: `rhythmCruiseEditionHome`）を使って「どちらの版から来たか」を推定し、ホームへ戻るリンク先を `./index.html` か `./pro_r4m8k7n2q9x/index.html` のどちらかに自動解決する仕組みになっている（`normalizeEditionHome` / `resolveHomePath` という同名の関数が各静的ページに重複実装されている。共通化はされていない）。
