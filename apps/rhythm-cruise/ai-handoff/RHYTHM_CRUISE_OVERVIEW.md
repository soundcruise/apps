...
- 現在のバージョン: `1.0.4`（`script.js` の `RHYTHM_CRUISE_VERSION`。2026年7月8日付でVer 1.0.0として正式リリース）
...
- 「リズムを作る」画面の `PROカスタムSTAGE` 入口ボタン（v1.0.4で表示のみ変更）: 表示名を「PROカスタム」に短縮し、見た目を他STAGEボタンと同じ `.stage-card` デザイン＋`.pro-custom-home-badge`（保存済みカスタムSTAGEと同じPRO★バッジ）に統一。`id="cat-pro-custom"` とクリック導線（`openRhythmProCustom`）・PROロック仕様は変更なし。
