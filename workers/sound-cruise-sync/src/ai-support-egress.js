...
// A secret-like code or a full ID anywhere means the input is not sent. Only a yes/no leaves this
...
function safeValue(value, depth = 0) {
  ...
  if (typeof value === 'string') {
    ...
    if (trimmed.startsWith('{') || trimmed.startsWith('[')) {
      ...
      return safeValue(parsed, depth + 1);
    }
    return true;
  }
  ...
  if (typeof value === 'object') {
    return Object.entries(value).every(([key, inner]) => !containsSensitive(key) && safeValue(inner, depth + 1));
  }
...
// true → safe to send. false → the input carries secret-like text or a full ID and must not be sent.
export function isProviderInputSafe(input) {
  if (!input || typeof input !== 'object' || Array.isArray(input)) return false;
  if (!Array.isArray(input.messages)) return false;
  for (const [key, value] of Object.entries(input)) {
    if (key === 'messages') {
      if (!value.every((message) => trustedSystemMessage(message) || safeValue(message))) return false;
    } else if (key === 'tools') {
...
export function guardedProvider(provider, { maxCalls }) {
  ...
  return Object.freeze({
    ...
    async complete(request) {
      ...
      if (!isProviderInputSafe(request)) throw new EgressBlockedError();
      calls += 1;
