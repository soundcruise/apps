...
After Phase 2, never lower the generation, restore legacy compatibility, or return to an earlier code slot. Recovery must keep the server-verification model and use a forward fix or a compatible rollback that preserves the Pro Auth API and current generation.
