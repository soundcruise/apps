...
test('route gates separate admission, writes, reads, Recovery, deletion and device management', () => {
  ...
  assert.equal(gateDecision('/v1/sync/assets/content', emergency).code, 'sync_write_paused');
  assert.equal(gateDecision('/v1/sync/assets/commit', emergency).code, 'sync_write_paused');
  assert.equal(gateDecision('/v1/sync/assets/unreference', emergency).code, 'sync_write_paused');
  assert.equal(gateDecision('/v1/sync/assets/download', emergency).code, 'sync_read_paused');
